...
import { getDefaultRulesForNewPlan } from "@/data/predefined-plans";
...
export default function NewPlanPage() {
  ...
  const { valuesByOrgUnit: mortalityByOrgUnit } = useMetricValues(407);      // Mortalite infanto-juvenile
  const { valuesByOrgUnit: incidenceByOrgUnit } = useMetricValues(410);      // Incidence
  const { valuesByOrgUnit: resistanceByOrgUnit } = useMetricValues(412);     // Resistance aux insecticides
  const { valuesByOrgUnit: seasonalityByOrgUnit } = useMetricValues(413);    // Saisonnalite
  ...
  // Initialize rules with default rule for new plan
  useEffect(() => {
    if (
    ...
    const defaultRule = defaultRules.find((r) => r.isAllDistricts);
    if (defaultRule) {
      ...
      console.log("Initialized new plan with default rule");
    }
  ...
  const handleExportPlan = useCallback(() => {
    ...
    link.setAttribute("download", `intervention-plan-new-${date}.csv`);
