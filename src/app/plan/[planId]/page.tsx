...
import { getPlanById } from "@/data/predefined-plans";
...
export default function PlanPage() {
  ...
  const displayName = plan?.name ?? "Unknown Plan";

  // Initialize rules from predefined plan on load
  useEffect(() => {
    if (
    ...
    // Apply the default rule to all districts first
  ...
  }, [districts, interventionCategories, updateDistricts, plan]);
  ...
  const handleExportPlan = useCallback(() => {
    ...
    link.setAttribute("download", `intervention-plan-${planId}-${date}.csv`);
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  }, [districts, interventionCategories, planId]);
